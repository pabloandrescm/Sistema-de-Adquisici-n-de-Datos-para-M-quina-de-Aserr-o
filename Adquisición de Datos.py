...
PUERTO_ADAM, BAUD_ADAM           = "COM10", 9600
...
class MonitorIndustrial:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.AUTO_SCALE_WINDOW_S = 15  # Ventana de autoescalado (segundos recientes)
    ...
    def _generar_informe_pdf(self):
        ...
        def confirmar():
            ...
            try:
                ...
                win.grab_release()
                win.destroy()
                # cerrar también ventana de edición si está abierta
        ...
        def guardar():
            for nombre, ent in entradas.items():
                try:
                    ...
                    return
            # Actualizar el combobox de tarifas si existe
    ...
    def stop(self):
        ...
        self._add_charts(); self.xlwb.save(self.xl_path)
    ...
    def _add_charts(self):
        ...
        max_r = ws_d.max_row
        ts_ref = Reference(ws_d, 1, 1, 2, max_r)
        for col in range(2, ws_d.max_column + 1):
            hdr = ws_d.cell(1, col).value
            data = Reference(ws_d, col, col, 1, max_r)
            ch = LineChart()
            ch.title = hdr
            ch.y_axis.title = hdr
            ...
            ch.height = 7
            ch.width = 16
            ws_g.add_chart(ch, f"A{1 + (col - 2) * 15}")
    ...
    def _show_tooltip(self, event):
        ...
        ln, idx = best
        xv = ln.get_xdata()[idx]
        # Cambia aquí la leyenda de cada gráfico:
    ...
    def _crear_pdf(self, carpeta, tarifa):
        try:
            ...
            if hasattr(self, "t0_dt") and self.t0_dt and self.t:
                ...
                dur_texto = str(duracion).split(".")[0]  # quitar microsegundos
